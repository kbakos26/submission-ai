...
function SubmissionFlowContent() {
  ...
  const [scanningComplete, setScanningComplete] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (currentStepKey === 'extraction' && extractedData.length === 0 && documents.length > 0) {
      ...
      documentScanProgress.forEach((scan, idx) => {
        ...
        setTimeout(() => {
          setScanningComplete(prev => [...prev, scan.status]);
        }, totalDelay + scan.delay - 100);
      });
      setTimeout(() => {
        setScanningDoc(null);
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {currentStepKey === 'extraction' && <DataExtractionStep isProcessing={isProcessing} scanningDoc={scanningDoc} scanningComplete={scanningComplete} extractedData={extractedData} setExtractedData={setExtractedData} />}
...
function DataExtractionStep({ isProcessing, scanningDoc, scanningComplete, extractedData, setExtractedData }: any) {
        ...
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-3 mb-4">
              ...
              <span className="text-xl font-semibold text-[var(--text-primary)]">AI Processing Documents...</span>
            </div>
          </div>

          <div className="space-y-3">
            {documentScanProgress.map((scan, idx) => {
              const isComplete = scanningComplete.includes(scan.status);
              const isCurrent = scanningDoc === scan.status;

              return (
                <div
                  key={idx}
                  className={`flex items-center gap-3 p-4 rounded-lg transition-all ${
                    isComplete ? 'bg-[var(--success)]/10' : isCurrent ? 'bg-[var(--accent-glow)]' : 'bg-[var(--bg-primary)]'
                  }`}
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                    isComplete ? 'bg-[var(--success)] text-white' : isCurrent ? 'bg-[var(--accent)] text-white' : 'bg-[var(--border)] text-[var(--text-muted)]'
                  }`}>
                    {isComplete ? '✓' : idx + 1}
                  </div>
                  <span className={`font-medium ${isComplete || isCurrent ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}`}>
                    {scan.status}
                  </span>
                </div>
              );
      ...
      <div className="bg-[var(--bg-secondary)] rounded-lg border border-[var(--border)] p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-[var(--text-muted)]">Review AI-extracted fields and edit any flagged items</p>
          </div>
        </div>
      ...
      <div className="bg-[var(--bg-secondary)] rounded-lg border border-[var(--border)] p-6 shadow-sm">
        ...
        <div className="space-y-3">
          {requiredDocs.map((doc: RequiredDocument, idx: number) => (
            <div key={idx} className="flex items-center justify-between p-4 bg-[var(--bg-primary)] rounded-lg border border-[var(--border)]">
              <div className="flex items-center gap-3">
...
function AcordFormsStep({ formsApproved, setFormsApproved, showToast }: any) {
    ...
    <div className="space-y-6">
      <div className="bg-[var(--bg-secondary)] rounded-lg border border-[var(--border)] shadow-sm">
        <div className="p-6 border-b border-[var(--border)]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[var(--text-muted)]">Review AI-generated ACORD forms</p>
            </div>
            <button
        ...
        <div className="p-6 bg-white">
          {activeTab === 'acord125' && <Acord125Form />}
          {activeTab === 'acord126' && <Acord126Form />}
          {activeTab === 'acord140' && <Acord140Form />}
          {activeTab === 'acord130' && <Acord130Form />}
        </div>
      </div>
    </div>
  );
...
function Acord125Form() {
  const data = acordFormData.acord125;

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xs bg-[var(--accent)]/10 text-[var(--accent)] px-3 py-1 rounded-full font-semibold animate-sparkle">✨ AI Generated</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <FormSection title="Agency Information">
          ...
          <FormField label="City, State, ZIP" value={`${data.agency.city}, ${data.agency.state} ${data.agency.zip}`} />
        ...
        <FormSection title="Named Insured">
          <FormField label="Legal Name" value={data.namedInsured.name} autoFilled />
          <FormField label="DBA" value={data.namedInsured.dba} autoFilled />
          <FormField label="Mailing Address" value={data.namedInsured.mailingAddress} autoFilled />
          <FormField label="City, State, ZIP" value={`${data.namedInsured.city}, ${data.namedInsured.state} ${data.namedInsured.zip}`} autoFilled />
          <FormField label="Phone" value={data.namedInsured.phone} autoFilled />
          <FormField label="FEIN" value={data.namedInsured.fein} autoFilled />
          <FormField label="Entity Type" value={data.namedInsured.entityType} autoFilled />
        </FormSection>
      </div>

      <FormSection title="Business Information">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="NAICS Code" value={data.businessInfo.naicsCode} autoFilled />
          <FormField label="NAICS Description" value={data.businessInfo.naicsDescription} autoFilled />
          <FormField label="Years in Business" value={data.businessInfo.yearsInBusiness.toString()} autoFilled />
          <FormField label="Total Locations" value={data.businessInfo.totalLocations.toString()} autoFilled />
          <FormField label="Total Employees" value={data.businessInfo.totalEmployees.toString()} autoFilled />
          <FormField label="Annual Revenue" value={`$${data.businessInfo.totalAnnualRevenue.toLocaleString()}`} autoFilled />
        </div>
        <FormField label="Description of Operations" value={data.businessInfo.descriptionOfOperations} autoFilled multiline />
      </FormSection>

      <FormSection title="Lines of Business Requested">
        <div className="grid grid-cols-2 gap-3">
          {data.linesRequested.map((line, idx) => (
      ...
      <FormSection title="Prior Carrier Information">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Prior Carrier" value={data.priorCarrier.name} autoFilled />
          <FormField label="Policy Number" value={data.priorCarrier.policyNumber} autoFilled />
          <FormField label="Expiration Date" value={data.priorCarrier.expirationDate} autoFilled />
          <FormField label="Total Premium" value={`$${data.priorCarrier.totalPremium.toLocaleString()}`} autoFilled />
          <FormField label="Years with Carrier" value={data.priorCarrier.yearsWithCarrier.toString()} autoFilled />
      ...
      <FormSection title="Loss History (Last 5 Years)">
        <div className="space-y-3">
          {data.lossHistory.map((claim, idx) => (
              ...
              <div className="grid grid-cols-5 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                    claim.status === 'Closed' ? 'bg-[var(--success)]/10 text-[var(--success)]' : 'bg-amber-50 text-amber-600'
                  }`}>
                    {claim.status}
                  </span>
                </div>
...
function Acord126Form() {
  const data = acordFormData.acord126;
    ...
    <div className="space-y-6">
      ...
      <FormSection title="Classification">
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Total Gross Receipts" value={`$${data.classification.grossReceipts.toLocaleString()}`} autoFilled />
          <FormField label="Liquor Receipts" value={`$${data.classification.liquorReceipts.toLocaleString()}`} autoFilled />
        </div>
      </FormSection>

      <FormSection title="Limits Requested">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Each Occurrence" value={`$${(data.limitsRequested.eachOccurrence / 1000000).toFixed(1)}M`} autoFilled />
          <FormField label="General Aggregate" value={`$${(data.limitsRequested.generalAggregate / 1000000).toFixed(1)}M`} autoFilled />
          <FormField label="Products/Completed Ops Agg" value={`$${(data.limitsRequested.productsCompletedOpsAggregate / 1000000).toFixed(1)}M`} autoFilled />
          <FormField label="Personal & Advertising Injury" value={`$${(data.limitsRequested.personalAdvertisingInjury / 1000000).toFixed(1)}M`} autoFilled />
          <FormField label="Damage to Rented Premises" value={`$${(data.limitsRequested.damageToRentedPremises / 1000).toFixed(0)}K`} autoFilled />
          <FormField label="Medical Expense" value={`$${(data.limitsRequested.medicalExpense / 1000).toFixed(0)}K`} autoFilled />
        </div>
      </FormSection>
      ...
      <FormSection title="Liquor Liability Coverage">
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Liquor Sales %" value={`${data.liquorLiability.liquorSalesPercentage}%`} autoFilled />
          <FormField label="Each Occurrence" value={`$${(data.liquorLiability.eachOccurrence / 1000000).toFixed(1)}M`} autoFilled />
          <FormField label="Aggregate" value={`$${(data.liquorLiability.aggregate / 1000000).toFixed(1)}M`} autoFilled />
        </div>
      </FormSection>

      <FormSection title="Additional Coverages">
        <div className="space-y-2">
          {data.additionalCoverages.map((coverage, idx) => (
            <div key={idx} className="flex items-center gap-2">
...
function Acord140Form() {
  const data = acordFormData.acord140;
    ...
    <div className="space-y-6">
      ...
      <FormSection title="Property Summary">
        <div className="grid grid-cols-3 gap-4">
          <FormField label="Total Building Value" value={`$${data.totalBuildingValue.toLocaleString()}`} autoFilled />
          <FormField label="Total Contents Value" value={`$${data.totalContentsValue.toLocaleString()}`} autoFilled />
          <FormField label="Business Income Limit" value={`$${data.totalBILimit.toLocaleString()}`} autoFilled />
          <FormField label="Deductible" value={`$${data.deductible.toLocaleString()}`} />
            ...
            <div key={idx} className="p-4 bg-[var(--bg-primary)] rounded-lg border border-[var(--border)]">
              ...
              <div className="grid grid-cols-6 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-semibold text-[var(--text-primary)]">${(loc.buildingValue / 1000).toFixed(0)}K</div>
                </div>
                <div>
                  <div className="text-xs text-[var(--text-muted)] mb-1">Contents</div>
                  <div className="font-semibold text-[var(--text-primary)]">${(loc.contentsValue / 1000).toFixed(0)}K</div>
                </div>
                <div>
...
function Acord130Form() {
  const data = acordFormData.acord130;
    ...
    <div className="space-y-6">
      ...
      <FormSection title="State & Coverage">
        <div className="grid grid-cols-3 gap-4">
          ...
          <FormField label="Total Payroll" value={`$${data.totalPayroll.toLocaleString()}`} autoFilled />
      ...
      <FormSection title="Classification Codes">
        <div className="space-y-3">
          {data.classificationCodes.map((classCode, idx) => (
            <div key={idx} className="p-4 bg-[var(--bg-primary)] rounded-lg border border-[var(--border)]">
              <div className="grid grid-cols-5 gap-4 text-sm">
                <div>
                ...
                <div>
                  ...
                  <div className="font-semibold text-[var(--text-primary)]">${classCode.payroll.toLocaleString()}</div>
                </div>
                <div>
      ...
      <FormSection title="Premium Summary">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Total Estimated Premium" value={`$${data.totalPremium.toLocaleString()}`} autoFilled />
          <FormField label="Deductible" value={data.deductible === 0 ? 'None' : `$${data.deductible.toLocaleString()}`} />
        </div>
      </FormSection>
    </div>
  );
...
function SubmissionPackageStep({ showToast }: any) {
  ...
  return (
    <div className="space-y-6">
              ...
              {file.type === 'folder' ? (
                <>
                  ...
                  >
                    ...
                    <span className="ml-auto text-[var(--text-muted)] text-sm">{file.children?.length} files</span>
                  </button>
                  {expandedFolders.includes(file.name) && file.children && (
                    <div className="ml-8 mt-2 space-y-2">
                      {file.children.map((child, childIdx) => (
                        <div key={childIdx} className="flex items-center gap-3 p-3 bg-white rounded border border-[var(--border)]">
      ...
      <div className="bg-[var(--bg-secondary)] rounded-lg border border-[var(--border)] p-6 shadow-sm">
        <h3 className="text-lg font-semibold mb-4 text-[var(--text-primary)]">Cover Letter Preview</h3>
...
function FormField({ label, value, autoFilled, multiline }: { label: string; value: string; autoFilled?: boolean; multiline?: boolean }) {
  return (
    <div>
      <div className="flex items-center gap-2 mb-1.5">
        <label className="text-xs font-medium text-[var(--text-muted)]">{label}</label>
        {autoFilled && (
          <span className="text-xs bg-[var(--accent)]/10 text-[var(--accent)] px-2 py-0.5 rounded font-medium">auto-filled</span>
        )}
      </div>
      {multiline ? (
        <textarea
...
export default function SubmissionFlowPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center"><div className="text-xl">Loading...</div></div>}>
